 ...
 */
//
// Modifications made by Shane Zabel 
// Shane Zabel modifications are commented with // instead of /*
//
public class RouteFindingAgentApp extends SimpleAgentApp {
	...
	protected static class RouteFindingAgentFrame extends MapAgentFrame {
		...
		/** Creates a new frame. */
// Modified to include "Romania, from Oradea"
// Modified to include new Heuristic
		public RouteFindingAgentFrame() {
			...
			setSelectorItems(SCENARIO_SEL, new String[] {
					...
					"Romania, from Fagaras", "Romania, from Oradea",
					"Australia, from Sydney",
					"Australia, from Random" }, 0);
			setSelectorItems(SEARCH_MODE_SEL, SearchFactory.getInstance()
					.getSearchModeNames(), 1);
			setSelectorItems(HEURISTIC_SEL, new String[] { "=0", "SLD", "SQRT(SLD)" }, 1);
		}
		...
		protected void selectionChanged(String changedSelector) {
			...
			RouteFindingAgentFrame.MapType mtype = (scenarioIdx < 4) ? MapType.ROMANIA
					: MapType.AUSTRALIA;
			if (mtype != usedMap) {
				usedMap = mtype;
				String[] items = null;
...
// Modified to include case 3 ... "Romania, from Oradea"
	protected static class RouteFindingAgentController extends
		...
		protected void selectScenarioAndDest(int scenarioIdx, int destIdx) {
			...
			case 2:
				...
				break;
// Modified to include case 3 ... "Romania, from Oradea"
// Updated other case values as needed
			case 3:
				SimplifiedRoadMapOfPartOfRomania.initMap(map);
				agentLoc = SimplifiedRoadMapOfPartOfRomania.ORADEA;
				break;
			case 4:
...
// Returns the trivial zero function, a simple heuristic which is
// based on straight-line distance computation, or a new heuristic 
// Modified to include case 2 ... the new Heuristic
		@Override
...
// Added the new Heuristic
// This Heuristic is the sqrt of the SLD heuristic
//
	static class H3 extends AdaptableHeuristicFunction {
		...
		public double h(Object state) {
			...
			Point2D pt2 = map.getPosition((String) goal);
			if (pt1 != null && pt2 != null)
				result = java.lang.Math.sqrt(pt1.distance(pt2));
			return result;
		}
	}
